...
|-------|---------|---------|
| Experiencia de incorporación | No se puede realizar la experiencia de incorporación a menos que se configure la red Wi-Fi del dispositivo (error de inicio de sesión de Azure). | 1. [SSH en Azure Percept DK](./how-to-ssh-into-percept-dk.md). <br> 2. Identificar y copiar la dirección IP ethernet del dispositivo. <br> 3. Conectarse a la experiencia de incorporación mediante la dirección URL basada en la dirección IP ethernet. |
| Experiencia de incorporación | A veces, al hacer clic en los vínculos del CLUF (contrato de licencia), no se abre una nueva página web. | Copie el vínculo y ábralo en una ventana del explorador independiente. |
...
| Wi-Fi | A veces, SoftAP puede desconectarse o desaparecer. | Estamos investigando el problema.  Al reiniciar el dispositivo, lo normal es que vuelva a aparecer. |
...
| Wi-Fi | Los usuarios pueden ver un mensaje después de conectarse a la red Wi-Fi: <br> "Esta red Wi-Fi usa un estándar de seguridad antiguo". | El punto SoftAP del kit de desarrollo usa el algoritmo de cifrado WEP. |
...
| Actualización de dispositivos | Después de realizar una actualización de OTA, se pierden los grupos de actualización. | Actualice la etiqueta del dispositivo siguiendo [estas instrucciones](./how-to-update-over-the-air.md#create-a-device-update-group). |
...
| Docker, red, IoT Edge | Si la red interna usa 172.x.x.x, los contenedores de Docker no podrán conectarse a IoT Edge. | Agregue una sección especial bip al archivo /etc/docker/daemon.json de la siguiente manera: `{    "bip": "192.168.168.1/24"}` |
