...
Azure Media Services admite los protocolos HLS, MPEG DASH y Smooth Streaming. Como parte del [empaquetado dinámico](encode-dynamic-packaging-concept.md), los manifiestos de cliente de streaming (HLS Master Playlist, DASH Media Presentation Description [MPD] y Smooth Streaming), se generan dinámicamente a partir del selector de formato de la dirección URL. Para más información, consulte los protocolos de entrega en el [flujo de trabajo común a petición](encode-dynamic-packaging-concept.md#to-prepare-your-source-files-for-delivery).
